...
const CATEGORIES: Category[] = [
  { id: 'destaque',  label: 'Em Destaque', count: 6, Icon: Flame,      color: '#f59e0b', glow: 'rgba(245,158,11,0.3)',  description: 'Ofertas exclusivas da semana' },
...
const CHESTS: StoreItem[] = [
  ...
  { id: 'ch4', name: 'Baú do Passe', rarity: 'EXCLUSIVO', price: 'PASSE', accent: '#ff004c', description: 'Recompensas exclusivas disponíveis apenas para detentores do Passe Ace.', chestGradient: 'linear-gradient(155deg,#3f0018 0%,#880025 60%,#cc0038 100%)', chestIcon: '🎴', badge: 'EXCLUSIVO' },
];
...
/* ── Edge highlight layers (glass block style) ──────────────────── */
...
function ChestPreview({ gradient, icon }: { gradient: string; icon: string }) {
  return (
    <div className="w-full h-full flex items-center justify-center" style={{ background: gradient }}>
      <span className="text-[44px] leading-none" style={{ filter: 'drop-shadow(0 0 16px rgba(255,255,255,0.35))' }}>
        {icon}
      </span>
    </div>
  );
...
/* ── Store card (2-column mobile grid) ─────────────────────────── */
function StoreCard({ item, index, isHero = false, onBuy }: {
  item: StoreItem; index: number; isHero?: boolean; onBuy: (item: StoreItem) => void;
}) {
  ...
  const metallicInsets = tier >= 5
    ...
    : tier >= 4
      ? 'inset 0 1px 0 rgba(255,255,255,0.48), inset 0 -1px 0 rgba(0,0,0,0.45), inset 1px 0 0 rgba(255,255,255,0.16)'
      : tier >= 3
        ? 'inset 0 1px 0 rgba(255,255,255,0.32), inset 0 -1px 0 rgba(0,0,0,0.35)'
        : tier >= 2
  ...
  const outerGlow = tier >= 5
    ...
    : tier >= 4
      ? `0 0 24px ${rarityColor}25, 0 10px 36px rgba(0,0,0,0.66)`
      : tier >= 3
        ? `0 0 16px ${rarityColor}18, 0 8px 32px rgba(0,0,0,0.62)`
        : '0 6px 24px rgba(0,0,0,0.55)';

  const cardShadow = [outerGlow, metallicInsets].filter(Boolean).join(', ');

  const stripH = tier >= 5 ? '4px' : tier >= 3 ? '3px' : '2px';
  const stripGlow = tier >= 5
    ? `0 0 14px ${rarityColor}, 0 0 28px ${rarityColor}70`
    : tier >= 4
      ? `0 0 9px ${rarityColor}cc`
      : tier >= 3
        ? `0 0 6px ${rarityColor}99`
        : 'none';

  const imgH = isHero ? 170 : 120;
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-[20px] overflow-hidden flex flex-col cursor-pointer${isHero ? ' col-span-2' : ''}`}
      onClick={() => onBuy(item)}
      style={{ background: cardBg, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: cardBorder, boxShadow: cardShadow }}
    ...
    >
      ...
      <div className="relative overflow-hidden shrink-0"
        style={{ height: `${imgH}px`, background: `linear-gradient(145deg, ${item.accent}22 0%, rgba(8,8,12,0.9) 100%)` }}>
        {item.image && (
          <>
          ...
          <div className="w-full h-full flex items-center justify-center"
            ...
            <div className="relative">
              ...
              <img src={item.avatarUrl} alt={item.name}
                className={`rounded-full relative z-10 ${isHero ? 'w-[100px] h-[100px]' : 'w-[68px] h-[68px]'}`}
      ...
      <div className="flex flex-col flex-1 px-3 pt-2.5 pb-3 gap-1">
        <span className="text-[9px] font-black uppercase tracking-[0.18em]" style={{ color: rarityColor }}>
          {item.rarity}
        </span>
        <h3 className="font-display leading-none tracking-wide"
          style={{ fontSize: isHero ? '22px' : '18px', color: 'rgba(255,255,255,0.92)' }}>
          {item.name}
        </h3>
        {isHero && item.description && (
          <p className="text-[10px] leading-[1.5] line-clamp-2 mt-0.5" style={{ color: 'rgba(255,255,255,0.32)' }}>
            {item.description}
          </p>
        )}
        {/* Price */}
        <div className="flex items-center gap-1.5 mt-auto pt-1.5">
          {isPasse ? (
            <span className="text-[10px] font-black uppercase tracking-wider" style={{ color: '#ff004c' }}>Ver Passe</span>
          ) : (
            <>
              <div className="w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0"
                style={{ background: 'linear-gradient(145deg,#00e870,#00a84a)', boxShadow: '0 0 6px rgba(0,210,106,0.55)' }}>
                <span className="leading-none font-black text-black" style={{ fontSize: '5px' }}>$</span>
              </div>
              <span className="font-black leading-none"
                style={{ fontSize: isHero ? '15px' : '13px', color: '#00e870', textShadow: '0 0 10px rgba(0,210,106,0.4)' }}>
                {item.price}
              </span>
            </>
          )}
      ...
      {/* Badge */}
      {item.badge && (
        <div className="absolute top-3 right-3 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider z-20"
...
function ConfirmModal({ item, onConfirm, onCancel }: {
  item: StoreItem; onConfirm: () => void; onCancel: () => void;
}) {
  const rarityColor = RARITY_COLOR[item.rarity] ?? '#9ca3af';
  const isPasse = item.price === 'PASSE';
      ...
      >
        ...
        <div className="flex flex-col px-5 pb-8 gap-4 overflow-y-auto no-scrollbar">
          ...
          <div className="relative rounded-[16px] overflow-hidden shrink-0"
            ...
            {/* Rarity strip on image */}
            <div className="absolute top-0 inset-x-0 h-[3px] rounded-t-[16px]"
            ...
            {!isPasse ? (
              <>
                ...
                <span className="text-[28px] font-black leading-none"
                  style={{ color: '#00e870', textShadow: '0 0 16px rgba(0,210,106,0.45)' }}>
                  {item.price}
                </span>
...
/* ── Category tab pill ──────────────────────────────────────────── */
function CategoryTab({ cat, isActive, onClick }: { cat: Category; isActive: boolean; onClick: () => void }) {
  const { Icon } = cat;
  return (
    <motion.button
      onClick={onClick}
      whileTap={{ scale: 0.94 }}
      className="relative flex items-center gap-2 px-4 py-2.5 rounded-[14px] overflow-hidden shrink-0"
      style={
        isActive
          ? {
              background: `linear-gradient(135deg, ${cat.color}1e 0%, ${cat.color}0a 100%)`,
              border: `1px solid ${cat.color}45`,
              boxShadow: `0 0 20px ${cat.color}1e, 0 4px 16px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.12)`,
            }
          : {
              background: 'linear-gradient(155deg, rgba(12,12,18,0.95) 0%, rgba(5,5,8,0.98) 100%)',
              border: '1px solid rgba(255,255,255,0.09)',
              boxShadow: '0 4px 14px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.08), inset 1px 0 0 rgba(255,255,255,0.04)',
            }
      }
    >
      ...
      {isActive && (
        <motion.div
          ...
          className="absolute bottom-0 inset-x-[20%] h-[2px] rounded-t-full z-10"
          style={{ background: cat.color, boxShadow: `0 0 8px ${cat.color}` }}
          transition={{ type: 'spring', stiffness: 400, damping: 32 }}
        />
      )}

      <Icon size={15} strokeWidth={2} color={isActive ? cat.color : 'rgba(255,255,255,0.3)'} className="relative z-10 shrink-0" />

      <span className="font-display leading-none relative z-10"
        style={{ fontSize: '15px', letterSpacing: '0.06em', color: isActive ? cat.color : 'rgba(255,255,255,0.35)' }}>
        {cat.label}
      </span>

      {cat.isNew && (
        <div className="relative z-10 shrink-0 w-2 h-2">
          <motion.div
...
export default function Store() {
  ...
  const handleBuy = (item: StoreItem) => setConfirmItem(item);
  ...
  return (
    <div className="h-full flex flex-col select-none relative overflow-hidden">
      ...
      <div className="shrink-0 flex gap-2 px-3 pt-3 pb-2 overflow-x-auto no-scrollbar">
        {CATEGORIES.map(cat => (
          <CategoryTab
            key={cat.id}
            cat={cat}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-baseline gap-2.5">
            <h2 className="font-display leading-none"
              style={{
                ...
                textShadow: `0 0 30px ${activeCat.color}55, 0 2px 8px rgba(0,0,0,0.8)`,
              }}>
              {activeCat.label}
            </h2>
          ...
          <div className="mt-1.5 h-[2px] rounded-full"
            style={{
              width: '38%',
              background: `linear-gradient(90deg, ${activeCat.color}80, ${activeCat.color}18, transparent)`,
            }} />
        </motion.div>
      </AnimatePresence>

      {/* ── Cards — 2-column grid, vertical scroll ─── */}
      <AnimatePresence mode="wait">
        <motion.div
          key={activeTab + '-grid'}
          initial={{ opacity: 0 }}
          ...
          exit={{ opacity: 0 }}
          transition={{ duration: 0.2 }}
          className="flex-1 overflow-y-auto no-scrollbar px-3 pb-8"
        >
